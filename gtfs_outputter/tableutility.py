...
class TableUtility:
    ...
    def generate_table(self, table_name, table, setup_row_func, rows=None,
        ...
        from a local file. Otherwise, the table will be generated by running the
        setup_row_func over the initial data rows or real-time GTFS feed
        ...
        if dataframeutility.can_read_dataframe(table_name, self.LOGIN, self.is_local, self.pathname) and not self.should_refresh:
            ...
            dataframeutility.write_dataframe(self.tables[
                                             table_name], table_name, self.LOGIN, self.is_local, self.pathname, self.should_refresh)
    ...
    def agency(self):
        ...
        table = pd.DataFrame(
            index=np.r_[0:len(self.static_feed['agency'].index)], columns=columns)

        def agency_row_func(i, row):
            # the ith row of the empty DataFrame passed in
            ...
            new_row['agency_lang'] = dataframeutility.optional_field(
                i, 'agency_lang', self.static_feed['agency'])
            new_row['agency_phone'] = dataframeutility.optional_field(
                i, 'agency_phone', self.static_feed['agency'])
    ...
    def routes(self):
        ...
        def routes_row_func(i, row):
            ...
            for direction_id in self.static_feed['trips'].loc[self.static_feed['trips']['route_id'] == row['route_id']]['direction_id'].unique():
                new_row = {}
                new_row['agency_id'] = self.agencyID
                new_row['route_short_name'] = str(dataframeutility.optional_field(i, 'route_short_name', self.static_feed[
                                                  'routes'], self.static_feed['routes'].iloc[i]['route_long_name']))
                new_row['route_dir'] = direction_id
                new_row['route_type'] = int(row['route_type'])
                new_row['route_long_name'] = str(dataframeutility.optional_field(i, 'route_long_name', self.static_feed[
                                                 'routes'], self.static_feed['routes'].iloc[i]['route_short_name']))
                new_row['route_desc'] = dataframeutility.optional_field(
                    i, 'route_desc', self.static_feed['routes'])
                new_row['route_url'] = dataframeutility.optional_field(
                    i, 'route_url', self.static_feed['routes'])
                new_row['route_color'] = dataframeutility.optional_field(
                    i, 'route_color', self.static_feed['routes'], default='FFFFFF').upper()
                new_row['route_text_color'] = dataframeutility.optional_field(
                    i, 'route_text_color', self.static_feed['routes'], default='000000').upper()
    ...
    def stops(self):
        ...
        columns = ['agency_id', 'stop_id', 'stop_code', 'stop_name',
                   ...
                   'version']
        table = pd.DataFrame(
            index=np.r_[0:len(self.static_feed['stops'].index)], columns=columns)
        ...
        def stops_row_func(i, row):
            ...
            new_row['stop_code'] = str(dataframeutility.optional_field(
                i, 'stop_code', self.static_feed['stops']))
            new_row['stop_name'] = str(row['stop_name'])
            new_row['stop_desc'] = str(dataframeutility.optional_field(
                i, 'stop_desc', self.static_feed['stops']))
            new_row['stop_lat'] = float(row['stop_lat'])
            new_row['stop_lon'] = float(row['stop_lon'])
            # some calculations, ignore until using MySQL
            new_row['lat_lon'] = 0
            new_row['stop_url'] = str(dataframeutility.optional_field(
                i, 'stop_url', self.static_feed['stops']))
            new_row['location_type'] = int(dataframeutility.optional_field(
                i, 'location_type', self.static_feed['stops'], 0))
            new_row['parent_station'] = int(dataframeutility.optional_field(
                i, 'parent_station', self.static_feed['stops'], 0))
            new_row['wheelchair_boarding'] = int(dataframeutility.optional_field(
                i, 'wheelchair_boarding', self.static_feed['stops'], 0))
            # TODO(erchpito) should be checksum
            new_row['version'] = 1
    ...
    # RESPONSE: Use BART's dataset, make a warning whenever shape_id is not
    ...
    def route_stop_seq(self):
        ...
        def route_stop_seq_row_func(i, row):
            ...
            for _, subrow in self.static_feed['trips'].loc[self.static_feed['trips']['route_id'] == route_id].iterrows():
                ...
                trip_id_block = self.static_feed['stop_times'].loc[self.static_feed[
                    'stop_times']['trip_id'] == trip_id]  # all the stops for a given trip_id
                ...
                sequence = trip_id_block['stop_id'].tolist().append(subrow[
                    'shape_id'])
                if str(sequence) not in patterns:
                    patterns += [str(sequence)]
                pattern_num = patterns.index(str(sequence)) + 1
                route_short_name = str(dataframeutility.optional_field(i, 'route_short_name', self.static_feed[
                                       'routes'], self.static_feed['routes'].iloc[i]['route_long_name']))
                pattern_id = "{0}_{1}_{2}".format(
                    route_short_name, direction_id, pattern_num)
                ...
                for k, subsubrow in trip_id_block.iterrows():
                    ...
                    new_row['is_time_point'] = int(dataframeutility.optional_field(
                        k, 'timepoint', self.static_feed['stop_times'], 0))
                    # TODO(erchpito) should be checksum
                    new_row['version'] = 1
                    self.tables[table_name] = self.tables[table_name].append(
                        pd.Series(new_row), ignore_index=True)
    ...
    def run_pattern(self):
        ...
        table = pd.DataFrame(
            index=np.r_[0:len(self.static_feed['trips'].index)], columns=columns)
        ...
        def runPattern_row_func(i, row):
            ...
            new_row['agency_id'] = self.agencyID
            j = np.where(self.static_feed['routes']['route_id'] == row['route_id'])[
                0][0]  # the index of the row containing the route
            new_row['route_short_name'] = str(dataframeutility.optional_field(j, 'route_short_name', self.static_feed[
                                              'routes'], self.static_feed['routes'].iloc[j]['route_long_name']))
            ...
            new_row['start_date'] = datetime.strptime(
                str(calendar['start_date']), "%Y%m%d")
            new_row['end_date'] = datetime.strptime(
                str(calendar['end_date']), "%Y%m%d")
            new_row['day'] = "{0}{1}{2}{3}{4}{5}{6}".format(calendar['monday'], calendar['tuesday'], calendar[
                                                            'wednesday'], calendar['thursday'], calendar['friday'], calendar['saturday'], calendar['sunday'])
            new_row['route_dir'] = int(dataframeutility.optional_field(
                i, 'direction_id', self.static_feed['trips'], 0))
            ...
            # RESPONSE: use route name
            run_key = '{0}_{1}_{2}'.format(new_row['route_short_name'], new_row[
                                           'route_dir'], new_row['day'])
            ...
            new_row['trip_headsign'] = dataframeutility.optional_field(i, 'trip_headsign', self.static_feed['trips'], self.static_feed[
                                                                       'stop_times'].loc[self.static_feed['stop_times']['trip_id'] == row['trip_id']]['stop_headsign'].iloc[0])
    ...
    def schedules(self):
        ...
        columns = ['agency_id', 'route_short_name', 'start_date', 'end_date',
                   ...
                   'version']
        table = pd.DataFrame(
            index=np.r_[0:len(self.static_feed['stop_times'].index)], columns=columns)
        ...
        def schedules_row_func(i, row):
            ...
            if row['trip_id'] not in runPattern_entry_memo:
                runPattern_entry_memo[row['trip_id']] = self.tables['RunPattern'].loc[
                    self.tables['RunPattern']['trip_id'] == row['trip_id']].iloc[0]
            ...
            route_stop_seq_entry = self.tables['Route_stop_seq'].loc[
                self.tables['Route_stop_seq']['stop_id'] == row['stop_id']].iloc[0]
            ...
            # RESPONSE: use Google's default
            new_row['pickup_type'] = int(dataframeutility.optional_field(
                i, 'pickup_type', self.static_feed['stop_times'], default=0))
            # TODO(erchpito) confirm default
            new_row['dropoff_type'] = int(dataframeutility.optional_field(
                i, 'drop_off_type', self.static_feed['stop_times'], default=0))
            new_row['arrival_time'] = row['arrival_time']
            new_row['departure_time'] = row['departure_time']
            # TODO(erchpito) how to do default NULL
            new_row['stop_headsign'] = str(dataframeutility.optional_field(
                i, 'stop_headsign', self.static_feed['stop_times']))
    ...
    def route_point_seq(self):
        ...
        columns = ['agency_id', 'route_short_name', 'route_dir', 'pattern_id',
                   ...
                   'version']
        table = pd.DataFrame(
            index=np.r_[0:len(self.static_feed['shapes'].index)], columns=columns)
        ...
        def route_point_seq_row_func(i, row):
            ...
            # Since many rows will require the same data from the route_stop_seq
            # table, this memoizes the entry needed based on the shape_id
            ...
            if row['shape_id'] not in route_stop_seq_entry_memo:
                ...
                route_stop_seq_entry_memo[row['shape_id']] = self.tables['Route_stop_seq'].loc[
                    self.tables['Route_stop_seq']['pattern_id'] == pattern_id].iloc[0]
                last_point = None
            route_stop_seq_entry = route_stop_seq_entry_memo[row['shape_id']]
            new_row['agency_id'] = route_stop_seq_entry['agency_id']
            new_row['route_short_name'] = route_stop_seq_entry[
                'route_short_name']
            ...
            points_entry = self.tables['Points'].loc[(self.tables['Points']['point_lat'] == row[
                                                      'shape_pt_lat']) & (self.tables['Points']['point_lon'] == row['shape_pt_lon'])].iloc[0]
            ...
            if last_point is None:
                ...
                # RESPONSE: use i - 1 to i
                new_row['heading'] = 0
            else:
                new_row['length'] = maputility.get_distance(last_point[0], last_point[1], row[
                                                            'shape_pt_lat'], row['shape_pt_lon'])
                total_dist += new_row['length']
                new_row['heading'] = maputility.get_heading(last_point[0], last_point[1], row[
                                                            'shape_pt_lat'], row['shape_pt_lon'])
            new_row['dist'] = total_dist
            last_point = (row['shape_pt_lat'], row['shape_pt_lon'])

        self.generate_table(
            table_name, table, route_point_seq_row_func, rows=self.static_feed['shapes'])
    ...
    def fare(self):
        ...
        """
        table_name = 'Fare'
        columns = ['agency_id', 'route_short_name', 'route_dir', 'pattern_id', 'price', 'currency_type',
                   'payment_method', 'origin_id', 'destination_id', 'transfers', 'transfer_duration', 'fare_id', 'version']
        table = pd.DataFrame()
    ...
    def calendar_dates(self):
        ...
        columns = ['agency_id', 'route_short_name', 'special_date',
                   'route_dir', 'run', 'exception_type', 'day', 'service_id', 'version']
        table = pd.DataFrame()
    ...
    # TODO(erchpito) consider a function that merely finds all legal transfer for a given stop via DataFrame calculation
    ...
    def transfers(self):
        ...
        def transfers_row_func(i, row):
            ...
            for _, subrow in self.tables['Route_stop_seq'].loc[self.tables['Route_stop_seq']['pattern_id'] != pattern_id].iterrows():
                ...
                # find mapping from stop_id to agency
                # generate intraagency and interagency indepedently
                ...
                (transfer_dist, min_transfer_time) = maputility.get_distance_and_time(
                    from_stop_point[0], from_stop_point[1], to_stop_point[0], to_stop_point[1])
                if transfer_dist > max_distance:
                    ...
                    if 'transfers' in self.static_feed and from_agency == to_agency:
                        transfers_entry = self.static_feed['transfers'].loc[(self.static_feed['transfers'][
                                                                             'from_stop_id'] == from_stop_id) & (self.static_feed['transfers']['to_stop_id'] == to_stop_id)]
                        other_transfers_entry = self.static_feed['transfers'].loc[(self.static_feed['transfers'][
                                                                                   'from_stop_id'] == to_stop_id) & (self.static_feed['transfers']['to_stop_id'] == from_stop_id)]
                        if not transfers_entry.empty:
                            transfer_type = transfers_entry.iloc[
                                0]['transfer_type']
                            min_transfer_time = int(dataframeutility.optional_field(
                                0, 'min_transfer_time', transfers_entry, min_transfer_time))
                        elif not other_transfers_entry.empty:
                            transfer_type = other_transfers_entry.iloc[
                                0]['transfer_type']
                            min_transfer_time = int(dataframeutility.optional_field(
                                0, 'min_transfer_time', other_transfers_entry, min_transfer_time))
    ...
    def transit_eta(self):
        ...
        def transit_eta_row_func(entity):
            ...
            s = datetime.strptime(gps_fixes_entry[
                                  'UTC_at_date'] + ' ' + gps_fixes_entry['UTC_at_time'], '%Y-%m-%d %H:%M:%S')
            timestamp = time.mktime(s.timetuple())

            # Given a Trip Update, find and iterate through the stops along that
            # trip, starting with the ith stop in the first Stop Time Update.
            ...
            while i < len(stop_time_updates):
                ...
                if 'departure' in stop_time_update or 'arrival' in stop_time_update:
                    ...
                    stop_seq_til = stop_time_updates[
                        i + 1]['stop_sequence'] if i + 1 < len(stop_time_updates) else len(trip_id_block) + 1

                    time_diff = stop_time_update[
                        'departure' if 'departure' in stop_time_update else 'arrival']
                    ...
                    while stop_seq < stop_seq_til:
                        ...
                        new_row['ETA'] = str(
                            (datetimeFromHMS(stop['departure_time']) + delay).strftime('%H:%M:%S'))
                        self.tables[table_name] = self.tables[table_name].append(
                            pd.Series(new_row), ignore_index=True)
                        stop_seq += 1
