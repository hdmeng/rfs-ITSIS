...
class TableUtility:
    ...
    def agency(self):
        ...
        def agency_row_func(i, row):
            new_row = self.tables[table_name].loc[i]  # the ith row of the empty DataFrame passed in
    ...
    def route_stop_seq(self):
        ...
        def route_stop_seq_row_func(i, row):
            ...
            for _, subrow in self.static_feed['trips'].loc[self.static_feed['trips']['route_id'] == route_id].iterrows():
                ...
                trip_id_block = self.static_feed['stop_times'].loc[self.static_feed['stop_times']['trip_id'] == trip_id]  # all the stops for a given trip_id
    ...
    def run_pattern(self):
        ...
        def runPattern_row_func(i, row):
            new_row = tables[table_name].loc[i]  # the ith row of the empty DataFrame passed in
            new_row['agency_id'] = self.agencyID
            j = np.where(self.static_feed['routes']['route_id'] == row['route_id'])[0][0]  # the index of the row containing the route
