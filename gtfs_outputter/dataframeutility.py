...
def df2sql(dataframe, df_name, login, exist_flag='append'):
    con = MySQLdb.connect(host=login['host'], user=login[
                          'user'], passwd=login['passwd'], db=login['db'])
    # seems to have no way to tell what types each column should be
    dataframe.to_sql(con=con, name=df_name, flavor='mysql',
                     if_exists=exist_flag, index=False)
    con.close()
...
def write_dataframe(dataframe, df_name, login, is_local, pathname, refresh):
    if is_local:
        ...
        df2sql(dataframe, df_name, login=login, exist_flag=(
            'replace' if refresh else 'append'))
