...
for i, row in st.iterrows():
    ...
    if row['route_id'] != route_id:
        ...
        if route_id not in route_names:
            ...
            route_names[route_id] = route_row[
                'route_short_name' if 'route_short_name' in route_row else 'route_long_name']
    ...
    pattern_id = "{0}_{1}".format(
        tag, patterns[tag].index(str(trip_stops)) + 1)
    ...
    for i in range(len(trip_stops)):
        ...
        new_row['seq'] = i + 1
        new_row['is_time_point'] = int(optional_field(
            stop_idx - len(trip_stops) + i, 'timepoint', sst, 0))
        new_row['version'] = checksum
        tables["Route_stop_seq"] = tables["Route_stop_seq"].append(
...
# probs not O(n) since it has to look for all the sequences before it goes
